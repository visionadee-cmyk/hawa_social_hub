        ...
        if (platform === 'facebook') {
          try {
            ...
            if (pagesData.data && pagesData.data.length > 0) {
              // Try to find Hawa Daily page first (case-insensitive, full phrase), otherwise use first page
              const hawaDailyPage = pagesData.data.find(p => p.name.toLowerCase().includes('hawa daily'));
          ...
          try {
            ...
            if (igData.data && igData.data.length > 0) {
              // Try to find Hawa Daily page first (case-insensitive, full phrase), otherwise use first page
              const hawaDailyPage = igData.data.find(p => p.name.toLowerCase().includes('hawa daily'));
